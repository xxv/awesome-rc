...
theme_path = "/home/steve/.config/awesome/theme.lua"
...
gui_file_browser = "gnome-open file:///home/steve/"
...
mytasklist.buttons = awful.util.table.join(
                     ...
                     awful.button({ }, 3, function ()
                                              if instance then
                                                  ...
                                                  instance = awful.menu.clients({ width=252 })
                                              end
                                          end),
...
--    awful.key({ modkey,           }, "w", function () mymainmenu:show(true)        end),
    ...
    awful.key({ modkey, "Control"   }, "space", function () awful.util.spawn(www_browser) end),
    awful.key({ modkey, "Shift"   }, "space", function () awful.util.spawn(gui_file_browser) end),
    ...
    awful.key({ modkey, "Control" }, "s",
              function ()
                  ...
                  mypromptbox[mouse.screen].widget,
                  function (host)
                      awful.util.spawn(terminal .. " -e ssh " ..host)
                  end, nil,
...
function bindtagkey(key, tagnum)
    globalkeys = awful.util.table.join(globalkeys,
        awful.key({ modkey }, key,
                      function ()
                            local screen = mouse.screen
                            if tags[screen][tagnum] then
                                awful.tag.viewonly(tags[screen][tagnum])
                            end
                      end),
            awful.key({ modkey, "Control" }, key,
                      function ()
                          local screen = mouse.screen
                          if tags[screen][tagnum] then
                              tags[screen][tagnum].selected = not tags[screen][tagnum].selected
                          end
                      end),
            awful.key({ modkey, "Shift" }, key,
                      function ()
                          if client.focus and tags[client.focus.screen][tagnum] then
                              awful.client.movetotag(tags[client.focus.screen][tagnum])
                          end
                      end),
            awful.key({ modkey, "Control", "Shift" }, key,
                      function ()
                          if client.focus and tags[client.focus.screen][tagnum] then
                              awful.client.toggletag(tags[client.focus.screen][tagnum])
                          end
                     end))
end
-- do the standard numerical bindings
...
-- continue with grid layout mappings
bindtagkey("'", 4)
bindtagkey(",", 5)
bindtagkey(".", 6)
